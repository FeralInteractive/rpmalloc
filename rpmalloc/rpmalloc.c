...
#define SPAN_LIST_LOCK_TOKEN      ((char*)1)
...
struct heap_t {
	...
	_Atomic(uintptr_t*) defer_deallocate;
...
//! Heap ID counter
static _Atomic int32_t _memory_heap_id;

#ifdef PLATFORM_POSIX
//! Virtual memory address counter
static _Atomic(char*) _memory_addr;
#endif

//! Global span cache
static _Atomic(char*) _memory_span_cache[SPAN_CLASS_COUNT];

//! Global large cache
static _Atomic(char*) _memory_large_cache[LARGE_CLASS_COUNT];

//! Current thread heap
static _Thread_local heap_t* _memory_thread_heap TLS_MODEL;

//! All heaps
static _Atomic(heap_t*) _memory_heaps[HEAP_ARRAY_SIZE];

//! Orphaned heaps
static _Atomic(heap_t*) _memory_orphan_heaps;

//! Active heap count
static _Atomic int32_t _memory_active_heaps;
...
static _Atomic int32_t _mapped_pages;
//! Running counter of total number of mapped memory pages since start
static _Atomic int32_t _mapped_total;
//! Running counter of total number of unmapped memory pages since start
static _Atomic int32_t _unmapped_total;
#endif
...
_memory_heap_lookup(int32_t id) {
	...
	heap_t* heap = atomic_load_explicit(&_memory_heaps[list_idx], memory_order_acquire);
...
_memory_global_cache_insert(span_t* first_span, size_t list_size, size_t page_count) {
...
#if MAX_SPAN_CACHE_DIVISOR > 0
	size_t class_idx = _span_class_from_page_count(page_count);
	char* global_span_ptr = atomic_load_explicit(&_memory_span_cache[class_idx],
	...
	while (1) {
		if (global_span_ptr != SPAN_LIST_LOCK_TOKEN) {
			...
			size_t cache_limit = GLOBAL_SPAN_CACHE_MULTIPLIER *
			                     (_memory_max_allocation[class_idx] / MAX_SPAN_CACHE_DIVISOR);
			...
			if (atomic_compare_exchange_weak_explicit(&_memory_span_cache[class_idx],
			                                          &global_span_ptr, first_span_ptr,
			...
			thread_yield();
			global_span_ptr = atomic_load_explicit(&_memory_span_cache[class_idx], memory_order_acquire);
		}
	}
#endif
...
_memory_global_cache_extract(size_t page_count) {
	...
	while (global_span_ptr) {
		if ((global_span_ptr != SPAN_LIST_LOCK_TOKEN) &&
		        atomic_compare_exchange_weak_explicit(&_memory_span_cache[class_idx],
		                                              &global_span_ptr, SPAN_LIST_LOCK_TOKEN,
		                                              memory_order_release, memory_order_acquire)) {
			...
			void* new_cache_head = global_span_count ?
			                       ...
			                       0;
			atomic_store_explicit(&_memory_span_cache[class_idx], new_cache_head, memory_order_release);
			break;
		}
...
#if MAX_SPAN_CACHE_DIVISOR > 0
	size_t class_idx = span_count - 1;
	char* global_span_ptr = atomic_load_explicit(&_memory_large_cache[class_idx], memory_order_acquire);
	while (1) {
		if (global_span_ptr != SPAN_LIST_LOCK_TOKEN) {
			uintptr_t global_list_size = (uintptr_t)global_span_ptr & ~SPAN_MASK;
			span_t* global_span = (span_t*)((void*)((uintptr_t)global_span_ptr & SPAN_MASK));
			...
			if (atomic_compare_exchange_weak_explicit(&_memory_large_cache[class_idx],
			                                          &global_span_ptr, new_global_span_ptr,
			                                          memory_order_release, memory_order_acquire))
				return;
		}
		else {
			//Atomic operation failed, yield timeslice and retry
			thread_yield();
			global_span_ptr = atomic_load_explicit(&_memory_large_cache[class_idx], memory_order_acquire);
		}
	}
#endif
...
_memory_global_cache_large_extract(size_t span_count) {
	...
	while (global_span_ptr) {
		if ((global_span_ptr != SPAN_LIST_LOCK_TOKEN) &&
		        atomic_compare_exchange_weak_explicit(&_memory_large_cache[class_idx],
		                                              &global_span_ptr, SPAN_LIST_LOCK_TOKEN,
		                                              memory_order_release, memory_order_acquire)) {
			uintptr_t global_list_size = (uintptr_t)global_span_ptr & ~SPAN_MASK;
			span = (span_t*)((void*)((uintptr_t)global_span_ptr & SPAN_MASK));
			...
			void* new_global_span_ptr = global_list_size ?
			                            ...
			                            0;
			atomic_store_explicit(&_memory_large_cache[class_idx], new_global_span_ptr, memory_order_release);
			break;
		}
...
_memory_allocate_from_heap(heap_t* heap, size_t size) {
	...
	const size_t class_idx = _memory_size_class[
	                             (size <= SMALL_SIZE_LIMIT) ?
	                             ((size + (SMALL_GRANULARITY - 1)) >> SMALL_GRANULARITY_SHIFT) - 1 :
	                             SMALL_CLASS_COUNT +
	                             ((size - SMALL_SIZE_LIMIT + (MEDIUM_GRANULARITY - 1)) >> MEDIUM_GRANULARITY_SHIFT) - 1
	                         ].class_idx;
...
use_active:
	...
	atomic_store_explicit(&span->heap_id, heap->id, memory_order_release);
	...
	//Track counters
	_memory_counter_increase(&heap->span_counter[span_class_idx],
	                         &_memory_max_allocation[span_class_idx]);
...
_memory_allocate_heap(void) {
	...
	do {
		heap = atomic_load_explicit(&_memory_orphan_heaps, memory_order_acquire);
		if (!heap)
			break;
		next_heap = heap->next_orphan;
	}
	while (!atomic_compare_exchange_weak_explicit(&_memory_orphan_heaps,
	                                              &heap, next_heap,
	                                              memory_order_release, memory_order_acquire));
	...
	//Get a new heap ID
	do {
		heap->id = atomic_fetch_add(&_memory_heap_id, 1);
	}
	while (!heap->id || _memory_heap_lookup(heap->id));

	//Link in heap in heap ID map
	size_t list_idx = heap->id % HEAP_ARRAY_SIZE;
	do {
		next_heap = atomic_load_explicit(&_memory_heaps[list_idx], memory_order_acquire);
		heap->next_heap = next_heap;
	}
	while (!atomic_compare_exchange_weak_explicit(&_memory_heaps[list_idx],
	                                              &next_heap, heap,
	                                              memory_order_release, memory_order_acquire));
...
_memory_deallocate_deferred(heap_t* heap, size_t size_class) {
	...
	uintptr_t* p = atomic_load_explicit(&heap->defer_deallocate, memory_order_acquire);
	if (!p)
		return 0;
	if (!atomic_compare_exchange_weak_explicit(&heap->defer_deallocate, &p, 0,
	                                           memory_order_release, memory_order_acquire))
		return 0;
	//Keep track if we deallocate in the given size class
	int got_class = 0;
	do {
		uintptr_t* next = (void*)(*p);
...
_memory_deallocate_defer(int32_t heap_id, void* p) {
	...
	uintptr_t* last_ptr = atomic_load_explicit(&heap->defer_deallocate, memory_order_acquire);
	do {
		*(uintptr_t*)p = (uintptr_t)last_ptr; //Safe to use block, it's being deallocated
	}
	while (!atomic_compare_exchange_weak_explicit(&heap->defer_deallocate, &last_ptr, p,
	                                              memory_order_release, memory_order_acquire));
}
...
_memory_allocate(size_t size) {
	...
	atomic_store_explicit(&span->heap_id, 0, memory_order_release);
...
_memory_deallocate(void* p) {
	...
	span_t* span = (void*)((uintptr_t)p & SPAN_MASK);
	int32_t heap_id = atomic_load_explicit(&span->heap_id, memory_order_acquire);
...
_memory_adjust_size_class(size_t iclass) {
	...
	page_count = ((page_count + (SPAN_CLASS_GRANULARITY - 1)) / SPAN_CLASS_GRANULARITY) *
	             SPAN_CLASS_GRANULARITY;
	if (page_count > 16)
		page_count = 16;
...
#else
#if ARCH_64BIT
	atomic_store_explicit(&_memory_addr, (char*)0x1000000000ULL, memory_order_release);
#else
	atomic_store_explicit(&_memory_addr, (char*)0x1000000ULL, memory_order_release);
#endif
#endif
	atomic_store_explicit(&_memory_heap_id, 0, memory_order_release);
...
rpmalloc_finalize(void) {
	...
	for (size_t list_idx = 0; list_idx < HEAP_ARRAY_SIZE; ++list_idx) {
		...
		atomic_store_explicit(&_memory_heaps[list_idx], 0, memory_order_release);
	}
	atomic_store_explicit(&_memory_orphan_heaps, 0, memory_order_release);

	//Free global caches
	for (size_t iclass = 0; iclass < SPAN_CLASS_COUNT; ++iclass) {
		void* span_ptr = atomic_load_explicit(&_memory_span_cache[iclass], memory_order_acquire);
		...
		atomic_store_explicit(&_memory_span_cache[iclass], 0, memory_order_release);
	}

	for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		void* span_ptr = atomic_load_explicit(&_memory_large_cache[iclass], memory_order_acquire);
		size_t cache_count = (uintptr_t)span_ptr & ~SPAN_MASK;
		span_t* span = (span_t*)((void*)((uintptr_t)span_ptr & SPAN_MASK));
		while (cache_count) {
			span_t* skip_span = span->prev_span;
		...
		atomic_store_explicit(&_memory_large_cache[iclass], 0, memory_order_release);
	}
}
...
rpmalloc_thread_initialize(void) {
	if (!_memory_thread_heap) {
		...
		atomic_fetch_add_explicit(&_memory_active_heaps, 1, memory_order_relaxed);
	}
}
...
rpmalloc_thread_finalize(void) {
	...
	heap_t* last_heap;
	do {
		last_heap = atomic_load_explicit(&_memory_orphan_heaps, memory_order_acquire);
		heap->next_orphan = last_heap;
	}
	while (!atomic_compare_exchange_weak_explicit(&_memory_orphan_heaps, &last_heap, heap,
	                                              memory_order_release, memory_order_acquire));
...
_memory_map(size_t page_count) {
...
#if ENABLE_STATISTICS
	atomic_fetch_add(&_mapped_pages, (int32_t)page_count, memory_order_relaxed);
	atomic_fetch_add(&_mapped_total, (int32_t)page_count, memory_order_relaxed);
#endif
	...
	if (total_size % SPAN_ADDRESS_GRANULARITY)
		++incr;
	do {
		void* base_addr = atomic_fetch_add_explicit(&_memory_addr,
		...
		if (pages_ptr != MAP_FAILED) {
			if (pages_ptr != base_addr) {
				...
				atomic_store_explicit(&_memory_addr, (char*)((uintptr_t)new_base) +
...
_memory_unmap(void* ptr, size_t page_count) {
#if ENABLE_STATISTICS
	atomic_add_explicit(&_mapped_pages, -(int32_t)page_count, memory_order_relaxed);
	atomic_add_explicit(&_unmapped_total, (int32_t)page_count, memory_order_relaxed);
#endif
...
rpmalloc_thread_statistics(rpmalloc_thread_statistics_t* stats) {
...
#endif
	void* p = atomic_load_explicit(&heap->defer_deallocate, memory_order_acquire);
	while (p) {
		void* next = *(void**)p;
		span_t* span = (void*)((uintptr_t)p & SPAN_MASK);
	...
	for (size_t isize = 0; isize < SPAN_CLASS_COUNT; ++isize) {
		if (heap->span_cache[isize])
			stats->spancache = (size_t)heap->span_cache[isize]->data.list_size * (isize + 1) *
			                   SPAN_CLASS_GRANULARITY * PAGE_SIZE;
	}
}
...
rpmalloc_global_statistics(rpmalloc_global_statistics_t* stats) {
...
#if ENABLE_STATISTICS
	stats->mapped = (size_t)atomic_load_explicit(&_mapped_pages, memory_order_relaxed) * PAGE_SIZE;
	stats->mapped_total = (size_t)atomic_load_explicit(&_mapped_total,
	                                                   memory_order_relaxed) * PAGE_SIZE;
	stats->unmapped_total = (size_t)atomic_load_explicit(&_unmapped_total,
	                                                     memory_order_relaxed) * PAGE_SIZE;
#endif
	for (size_t iclass = 0; iclass < SPAN_CLASS_COUNT; ++iclass) {
		void* global_span_ptr = atomic_load_explicit(&_memory_span_cache[iclass], memory_order_relaxed);
		while (global_span_ptr == SPAN_LIST_LOCK_TOKEN) {
			thread_yield();
			global_span_ptr = atomic_load_explicit(&_memory_span_cache[iclass], memory_order_relaxed);
		}
		uintptr_t global_span_count = (uintptr_t)global_span_ptr & ~SPAN_MASK;
	...
	for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		void* global_span_ptr = atomic_load_explicit(&_memory_large_cache[iclass], memory_order_relaxed);
		while (global_span_ptr == SPAN_LIST_LOCK_TOKEN) {
			thread_yield();
			global_span_ptr = atomic_load_explicit(&_memory_large_cache[iclass], memory_order_relaxed);
		}
		uintptr_t global_span_count = (uintptr_t)global_span_ptr & ~SPAN_MASK;
